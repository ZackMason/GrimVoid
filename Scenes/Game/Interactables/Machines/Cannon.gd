...
onready var ray = $turret_base_001/Pivot/FirePoint/RayCast2D

func fire():
	if ray.is_colliding() and ray.get_collider() == get_parent().get_parent():
		return
...
func _input(event):
	if Input.is_action_just_pressed("ui_cancel") and pilot:
